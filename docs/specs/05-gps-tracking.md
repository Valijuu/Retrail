...
`abstract interface class LocationSource { Stream<LocationFix> get fixes; Future<bool> ensurePermission(); }` + `GeolocatorLocationSource` impl: high-accuracy, ~1s interval; maps each `Position` to a `LocationFix`, **synthesizing the monotonic `elapsedRealtimeNanos`** (a `Stopwatch`-based clock) since geolocator only gives wall-clock — preserving the freshness filter's intent. Drives `RideTracker.onLocationReceived`.
