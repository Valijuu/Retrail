...
- Runner `macos-26` (Xcode 26 / iOS 26 SDK — required by current plugins); triggers: push to `main` and `phase/**` (so a phase branch is compile-checked before it lands) **when app code changed** (`lib/`, `ios/`, `assets/`, `pubspec.*`, `l10n.yaml`, the workflow itself) + `workflow_dispatch`. Also fails if the `.appex` was not embedded.
